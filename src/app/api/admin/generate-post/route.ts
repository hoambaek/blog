...
## 네 개의 기둥
1. 기록이 헤리티지다: 모든 병에는 입수일·좌표·수심·인양일과 바다에서 보낸 모든 날의 수온·해류가 기록되어 동봉됩니다.
...
2. 추상 대신 관측 수치: "오랜 시간" → "6개월~2년", "깊은 바다" → "수심 30m", "차가운 바다" → "평균 11.4°C", 위치는 "한국 남해". 감성 표현과 검증된 관측 데이터만 결합.
...
6. 금지 어휘 (2026-08-02 대표 확정) — 아래 넷은 어떤 경우에도 쓰지 않는다.
   - "심해": 실제 수심은 20~40m라 심해가 아니다. 깊이를 파는 말이기도 하다. 대신 "바다"(서사) 또는 "남해 수심 30m"(수치).
   - "퀴베" / "큐베": 제품 라인을 가리키는 자리는 전부 "에디션". 고유명 Édition Zéro만 프랑스어 원표기 유지.
   - "이 병" / "그 병" / "내 병": 개별 병을 대명사로 지칭하지 않는다. 개체를 지목해야 하면 번호로 부르고("N° 89의 기록"), 앞 문장이 대상을 이미 세웠으면 부르지 말고 기록을 주어로 세운다("바다에서 보낸 모든 날이 기록됩니다").
   - "보틀": 개체 지칭어로 쓰지 않는다. 형태·패키지를 말할 때만 허용.
7. "병"은 계속 쓴다. 바뀌는 것은 개체를 지목하는 자리뿐이다. 수량 단위("500병 한정", "전 병 개별 넘버링")와 물리적으로 담기는 자리("한 병의 샴페인이 바다 아래에서", "기록이 병과 함께 옵니다")는 정본이 허용하는 용법이다.`
